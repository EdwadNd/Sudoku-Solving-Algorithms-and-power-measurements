...
int main() {
    const string log_filename = "HardTest_log.csv";
    ...
    while (read_next_puzzle(file)) {
        ...
        //ULARGE_INTEGER startKernelTime, endKernelTime;
        QueryPerformanceCounter(&start_time);

        // Get initial CPU usage time
        FILETIME creationTime, exitTime, kernelTime, userTime;
        GetProcessTimes(GetCurrentProcess(), &creationTime, &exitTime, &kernelTime, &userTime);
        ULARGE_INTEGER startUserTime, startKernelTime;
        startUserTime.LowPart = userTime.dwLowDateTime;
        startUserTime.HighPart = userTime.dwHighDateTime;
        startKernelTime.LowPart = kernelTime.dwLowDateTime;
    startKernelTime.HighPart = kernelTime.dwHighDateTime;
        ...
        GetProcessTimes(GetCurrentProcess(), &creationTime, &exitTime, &kernelTime, &userTime);
        ULARGE_INTEGER endUserTime, endKernelTime;
        endUserTime.LowPart = userTime.dwLowDateTime;
        endUserTime.HighPart = userTime.dwHighDateTime;
        endKernelTime.LowPart = kernelTime.dwLowDateTime;
        endKernelTime.HighPart = kernelTime.dwHighDateTime;

// Calculate total CPU time
double cpu_time = ((endUserTime.QuadPart - startUserTime.QuadPart)/1e7 + 
                   (endKernelTime.QuadPart - startKernelTime.QuadPart)) / 1e7;
